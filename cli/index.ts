...
function printHelp() {
	...
	console.log(
		`  ${c.cyan}command${c.reset} | ${c.green}punt${c.reset}              Punt your output to the cloud`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.dim}<file>${c.reset}                 Punt a file (auto-detects language)`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--ttl 1h${c.reset}              Set expiry (default: 24h)`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--burn${c.reset}                Delete after first view`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--private${c.reset}             Require key to view`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--lang ts${c.reset}             Syntax highlight as TypeScript`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--show${c.reset} ${c.dim}<id>${c.reset}           Open paste in browser`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--cat${c.reset} ${c.dim}<url>${c.reset}           Fetch raw paste content`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}--delete${c.reset} ${c.dim}<id> <key>${c.reset}   Delete a paste`,
	);
	console.log();
	console.log(`${c.bold}ACCOUNT${c.reset}`);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}login${c.reset}              Sign in with GitHub`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}logout${c.reset}             Sign out and remove credentials`,
	);
	console.log(
		`  ${c.green}punt${c.reset} ${c.yellow}whoami${c.reset}             Show current user info`,
	);
	console.log();
	console.log(`${c.bold}OPTIONS${c.reset}`);
	console.log(
		`  ${c.yellow}--ttl${c.reset} ${c.dim}<duration>${c.reset}    Expiry time (e.g., 30m, 2h, 7d, 30d)`,
	);
	console.log(
		`  ${c.yellow}--burn${c.reset}             Burn after read (auto-delete on view)`,
	);
	console.log(
		`  ${c.yellow}--private${c.reset}          Private paste (generates view key)`,
	);
	console.log(
		`  ${c.yellow}--lang${c.reset} ${c.dim}<lang>${c.reset}       Syntax highlighting (js, ts, py, go, etc.)`,
	);
	console.log(
		`  ${c.yellow}--lang auto${c.reset}        Auto-detect language from filename`,
	);
	console.log(
		`  ${c.yellow}--show${c.reset} ${c.dim}<id>${c.reset}         Open paste in browser`,
	);
	console.log(
		`  ${c.yellow}--cat${c.reset} ${c.dim}<url|id>${c.reset}      Print raw paste to stdout`,
	);
	console.log(
		`  ${c.yellow}--delete${c.reset} ${c.dim}<id> <key>${c.reset} Delete paste with delete key`,
	);
	console.log(
	...
	console.log(
		`  ${c.green}punt${c.reset} secret.txt ${c.yellow}--burn${c.reset}`,
	);
	console.log();
	console.log(`  ${c.dim}# Private paste with view key${c.reset}`);
	console.log(
		`  ${c.cyan}echo "secret"${c.reset} | ${c.green}punt${c.reset} ${c.yellow}--private${c.reset}`,
	);
	console.log();
...
async function login(): Promise<void> {
	...
	if (existingToken) {
		console.log(
			`${c.yellow}You're already logged in. Run ${c.green}punt logout${c.yellow} first.${c.reset}`,
		);
		process.exit(1);
	}
	...
	for (let i = 0; i < maxPolls; i++) {
		...
		try {
			...
			if (data.status === "approved" && data.token) {
				...
				console.log();
				console.log(
					`Run ${c.cyan}punt whoami${c.reset} to see your account info.`,
				);
				console.log();
				process.exit(0);
...
async function createPaste(
	...
	if (size > 4 * 1024 * 1024) {
		printError(
			`Content too large (${(size / 1024 / 1024).toFixed(1)}MB). Max is 4MB.`,
		);
		process.exit(1);
	}
	...
	const spinText =
		flags.length > 0 ? `Punting (${flags.join(", ")})...` : "Punting...";
